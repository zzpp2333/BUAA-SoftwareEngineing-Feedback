...
def getCourses(request):
    if(request.method == 'GET'):
        ...
        res = {'num': len(data), 'courses': data, 'name': user.username, 'all_num': len(data_all), 'all_courses': data_all}
...
def getAssistApplications(request):
    if request.method == 'GET':
        ...
        for app in applications:
            app['no'] = UserProfile.objects.get(user_id=app['user_id']).no
...
def isYourCourse(request, course_id):
    if isinstance(course_id, str):
        course_id = int(course_id)
    user = request.user
    course_user = user.course_user
    courses = course_user.course.values('course_id')
    permission = user.profile.permission
    if permission == 4:
        return True
    ...
    for c in courses:
        if c['course_id'] == course_id:
            flag = 1
    # print(courses)
    if flag == 0 and permission == 3:
        assistant = user.course_assistant
        a_courses = assistant.course.values('course_id')
    ...
    if flag == 1:
        print(1)
        # return HttpResponse(json.dumps({'emm': True}), content_type='application/json')
        return True

    print(0)
    # return HttpResponse(json.dumps({'emm': False}), content_type='application/json')
    return False
